# 地表最强IDE：VSCode {#sec-ide}

## 背景
大毛最开始写代码时，用的是最朴素的办法：在终端里一个字一个字敲。但是他很快发现了这样的弊端，不管是查看文件树、写代码文件、还是运行，都要频繁地切换终端会话。

后来他发现，真正顺手的工作方式，应该把写代码、看文件、开终端、装插件、连服务器这些动作尽量放在同一个地方完成。那么，什么样的IDE最适合像他这样的生信初学者？
...
如图 @fig-vscode-interface 所示，VSCode 的主界面由活动栏、侧边栏、编辑区、底部面板和状态栏等部分组成。

![VSCode界面示意图](img/2.1.vscode-interface.png){#fig-vscode-interface}
...
- Github Copilot和GitHub Copilot Chat：微软官方的AI编程助手插件，顺手的时候可以用一下。如果您按照 @sec-other-ai 中的方法使用学生认证获得了GitHub Copilot的免费高级权限，就可以直接使用。
...
如图 @fig-vscode-settings 所示，VSCode 的设置界面可以直接搜索配置项，也可以切换到 JSON 方式精细修改。

![VSCode设置界面](img/2.1.vscode-settings.png){#fig-vscode-settings}

您可能会发现，VSCode和插件的配置有两套或三套体系：**用户级别**（User）、**机器级别**（Machine/Remote，如果您使用WSL或远程服务器的话）和**工作区级别**（Workspace）。一般来说，用户级别的配置适用于所有项目，机器级别的配置适用于特定的机器，而工作区级别的配置则只适用于当前打开的项目文件夹。配置的优先级为：**工作区 > 机器 > 用户**。因此您可以根据需要选择合适的级别进行配置，比如，如果您想让某个插件在所有项目中都生效，可以在用户级别进行配置；如果您只想让它在某个特定项目中生效，可以在工作区级别进行配置。
