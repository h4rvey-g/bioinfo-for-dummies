...
![VSCode设置界面](img/2.1.vscode-settings.png)

您可能会发现，VSCode和插件的配置有两套或三套体系：**用户级别**（User）、**机器级别**（Machine/Remote，如果你使用WSL或远程服务器的话）和**工作区级别**（Workspace）。一般来说，用户级别的配置适用于所有项目，机器级别的配置适用于特定的机器，而工作区级别的配置则只适用于当前打开的项目文件夹。配置的优先级为：**工作区 > 机器 > 用户**。因此您可以根据需要选择合适的级别进行配置，比如，如果你想让某个插件在所有项目中都生效，可以在用户级别进行配置；如果你只想让它在某个特定项目中生效，可以在工作区级别进行配置。
