...
8. 安装Zellij，一个终端复用器。学习如何使用Zellij进行终端窗口的分屏和管理。查看 @sec-zellij 中的讲解。
9. （如你在用计算集群）理解什么是HPC与SLURM；学会用`squeue/sinfo/scancel`查看与管理作业；能写一个最小可运行的`sbatch`脚本。查看 @sec-hpc-slurm。
...
按提示重启电脑。重启后，第一次打开 Ubuntu 会让你设置：

- Linux 用户名
- Linux 密码（输入时不会显示字符，这是正常的）

如果你想指定发行版（例如 Ubuntu），可以用：
...
如果你在国内网络环境，`apt update`/安装软件很慢，可以把 Ubuntu 的软件源切到清华 TUNA 镜像。
...
如果你想恢复原来的配置，把备份文件改回去即可（把日期换成你自己的）：

```bash
sudo mv /etc/apt/sources.list.bak.YYYY-MM-DD /etc/apt/sources.list
sudo apt update
```
...
用 VS Code 连上 WSL 后，你看到的是 Linux 环境：终端、路径、`apt`、`bash` 都是 WSL 里的，不是 Windows 的。
...
- `/`：根目录
- `/home`：用户家目录的上一级目录；你的家目录通常是 `/home/<用户名>`，也写作 `~`
- `./`：当前所在的目录
- `../`：当前所在目录的上一级目录
...
显而易见，一个文件/目录的绝对路径是唯一的，在任何位置都能访问到；而相对路径则依赖于你当前所在的位置。
...
**alias** 和 **PATH** 是两个非常重要的配置项。alias是命令的别名，可以让你用更短的命令来执行常用的操作；PATH是一个环境变量，告诉Shell在哪些目录下、以什么顺序查找软件。
...
类似于 Windows 里的“快捷方式”，Linux 里有**符号链接（symlink）**，可以让你用一个路径指向另一个路径。创建符号链接用 `ln -s`：

```bash
ln -s /data/big/project_data ~/projects/project_data
```

这样，你就可以通过 `~/projects/project_data` 来访问 `/data/big/project_data` 了。
...
端口转发可以让你通过 SSH 隧道访问远程服务器上的服务，例如 Jupyter Notebook、RStudio Server 等。例如，您在远程服务器上运行了一个 Jupyter Notebook 服务，监听在 8888 端口上，您可以通过以下命令将远程的 8888 端口映射到本地的 8888 端口：
...
- **计算节点（compute node）**：真正跑计算的地方。你必须通过SLURM申请资源后，才能在计算节点上运行耗时任务。
...
- **环境可复现**：不要依赖你“登录后手动source过”的状态。把`module load ...`或环境激活命令写进脚本（也可以在脚本里打印`which python`、`python -V`自检）。
