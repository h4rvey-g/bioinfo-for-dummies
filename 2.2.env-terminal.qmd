...
## 背景
某天，老师给了大毛一个服务器账号，让他把RNA-seq数据传上去处理。大毛满怀期待地连上服务器，屏幕上却只剩下一个黑底白字的终端和一个闪烁的光标。没有图标，没有鼠标点点点，甚至连“我的电脑”都找不到。他盯着屏幕看了半天，第一次真切地感受到什么叫“不会用服务器”。

可偏偏生信里最常见的软件、脚本和分析流程，几乎都活在Linux和终端里。大毛想知道：这个看起来很吓人的黑框，到底为什么这么重要？自己又该从哪里开始学起，才不至于一上来就把服务器弄坏？
...
`htop` 是一个交互式的进程查看器，可以实时显示系统资源使用情况和进程信息。安装后，直接运行 `htop` 即可。图 @fig-htop 展示了它的典型界面。

![htop截图](img/2.2.htop.png){#fig-htop}
